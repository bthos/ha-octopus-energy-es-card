...
export function createMockHass(overrides = {}) {
    const mockCallService = overrides.callService || (async (domain, service, serviceData) => {
 ...
 */
export function createMockConsumptionData() {
    return [
        {
 ...
 */
export async function waitForDataLoad(element, timeout = 500) {
    ...
    if (!element.shadowRoot && element.isConnected) {
        ...
        element.renderRoot = shadowRoot;
        // Enable updating
        if (typeof element.enableUpdating === 'function') {
            element.enableUpdating(true);
        }
        // Set isUpdatePending to false to allow updates
        element.isUpdatePending = false;
 ...
 */
export async function waitForStableState(element, maxAttempts = 20) {
 ...
 */
export function getPrivateProperty(element, property) {
