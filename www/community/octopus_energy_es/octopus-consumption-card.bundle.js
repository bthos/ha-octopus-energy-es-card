...
/**
  ...
  `,ut([pt({attribute:!1})],gt.prototype,"hass",void 0),ut([pt({attribute:!1})],gt.prototype,"config",void 0),ut([ht()],gt.prototype,"_config",void 0),"undefined"!=typeof window&&"undefined"!=typeof customElements&&(customElements.get("octopus-consumption-card-editor")||customElements.define("octopus-consumption-card-editor",gt));var mt=function(t,e,o,i){var r,s=arguments.length,n=s<3?e:null===i?i=Object.getOwnPropertyDescriptor(e,o):i;if("object"==typeof Reflect&&"function"==typeof Reflect.decorate)n=Reflect.decorate(t,e,o,i);else for(var a=t.length-1;a>=0;a--)(r=t[a])&&(n=(s<3?r(n):s>3?r(e,o,n):r(e,o))||n);return s>3&&n&&Object.defineProperty(e,o,n),n};void 0!==at&&at.disableWarning&&at.disableWarning("change-in-update");class yt extends at{constructor(){super(...arguments),this._consumptionData=[],this._comparisonResult=null,this._loading=!1,this._error=null,this._comparisonError=null,this._currentPeriod="week",this._currentDate=new Date,this.SERVICE_TIMEOUT=1e4}setConfig(t){if(!t)throw new Error("Invalid configuration");this.config=t}connectedCallback(){super.connectedCallback(),this._validateConfig(),this._currentPeriod=this.config.default_period||"week",this._loadData()}updated(t){super.updated(t),t.has("config")&&(this._validateConfig(),void 0!==t.get("config")&&this._loadData())}_validateConfig(){return this.config?this.config.source_entry_id?(!this.config.show_tariff_comparison||this.config.tariff_entry_ids&&0!==this.config.tariff_entry_ids.length||console.warn("show_tariff_comparison is enabled but no tariff_entry_ids provided. Comparison will be disabled."),void(this.config.show_cost_on_chart&&!this.config.selected_tariff_for_cost&&console.warn("show_cost_on_chart is enabled but selected_tariff_for_cost is not set. Cost display will be disabled."))):(this._error="source_entry_id is required. Please select your tariff from the card editor.",void(this._loading=!1)):(this._error="Card configuration is required",void(this._loading=!1))}_createTimeoutPromise(t){return new Promise((e,o)=>{setTimeout(()=>o(new Error("Request timeout. The service call took too long to respond.")),t)})}async _callServiceWithTimeout(t,e,o){try{const i={...o};i&&"return_response"in i&&delete i.return_response,console.log("%c  Calling service: %c"+t+"."+e,"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),console.log("%c  Service data: %c"+JSON.stringify(i,null,2),"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;");const r=this.hass.callService(t,e,i),s=this._createTimeoutPromise(this.SERVICE_TIMEOUT),n=await Promise.race([r,s]);return console.log("%c  Raw Service Response: %c"+JSON.stringify(n,null,2),"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),n}catch(o){if(console.error("%c  Service Error Details: %c"+JSON.stringify(o,Object.getOwnPropertyNames(o),2),"color: #f00; font-size: 11px;","color: #f00; font-size: 11px; font-family: monospace;"),o instanceof Error){if(o.message.includes("timeout"))throw new Error(`Service call timeout: ${t}.${e} took longer than ${this.SERVICE_TIMEOUT}ms`);if(o.message.includes("Service not found")||o.message.includes("not available"))throw new Error(`Service ${t}.${e} is not available. Please ensure the Octopus Energy España integration is installed and configured.`);if("service_validation_error"===o.code){const t=o;let e=t.message||t.translation_key||"Service validation error";throw e.includes("return_response")&&(e="The Octopus Energy España integration service does not support response data. This is a limitation of the integration. Please update the integration to the latest version, or contact the integration maintainer. The service may need to be updated to support response data properly."),new Error(`Service validation error: ${e}`)}throw new Error(`Service call failed: ${t}.${e} - ${o.message}`)}if(o&&"object"==typeof o){const i=o;let r=i.message||i.translation_key||"Unknown service error";throw"service_validation_error"===i.code&&r.includes("return_response")&&(r="The Octopus Energy España integration service does not support response data. This is a limitation of the integration. Please update the integration to the latest version, or contact the integration maintainer. The service may need to be updated to support response data properly."),new Error(`Service call failed: ${t}.${e} - ${r}`)}throw o}}async _loadData(){if(!this.hass||!this.config)return;const t=this.config.source_entry_id;if(!t)return this._error="source_entry_id is required. Please select your tariff from the card editor.",void(this._loading=!1);this._loading=!0,this._error=null,this._comparisonError=null;try{const{startDate:e,endDate:o}=this._getDateRange();if(e>new Date)throw new Error(`Invalid date range: start date (${e.toISOString().split("T")[0]}) is in the future. Please navigate to a past period.`);if(e>o)throw new Error("Invalid date range: start date is after end date.");let i,r;console.log("%cℹ Fetching consumption data","color: #666; font-size: 11px;"),console.log("%c  Entry ID: %c"+t+"%c | Period: %c"+this._currentPeriod+"%c | Dates: %c"+e.toISOString().split("T")[0]+" → "+o.toISOString().split("T")[0],"color: #666; font-size: 11px;","color: #999; font-size: 11px;","color: #666; font-size: 11px;","color: #999; font-size: 11px;","color: #666; font-size: 11px;","color: #999; font-size: 11px;");try{r=await this._callServiceWithTimeout("octopus_energy_es","fetch_consumption",{entry_id:t,start_date:e.toISOString().split("T")[0],end_date:o.toISOString().split("T")[0],granularity:"day"===this._currentPeriod||"week"===this._currentPeriod?"hourly":"daily"}),console.log("%c  Raw Service Response (before processing): %c"+JSON.stringify(r,null,2),"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),i=r}catch(t){let e,o;if(t instanceof Error)e=t.message,o=e;else if(t&&"object"==typeof t){const i=t;e=i.message||i.translation_key||JSON.stringify(t),o="service_validation_error"===i.code?e.includes("return_response")?"The Octopus Energy España integration service does not support response data. This is a limitation of the integration. Please update the integration to the latest version, or contact the integration maintainer. The service may need to be updated to support response data properly.":e||"Service validation error. Please check your configuration.":e}else e=String(t),o=e;throw console.error("%c✗ Service call failed: %c"+e,"color: #f00; font-size: 11px; font-weight: bold;","color: #f00; font-size: 11px;"),console.error("%c  Full Error Object: %c"+JSON.stringify(t,Object.getOwnPropertyNames(t),2),"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),new Error(o)}if(!i||"object"!=typeof i)throw console.error("%c✗ Invalid service response: %cexpected object with success field","color: #f00; font-size: 11px; font-weight: bold;","color: #f00; font-size: 11px;"),new Error("Invalid response from service: expected object with success field");if(!i.success){const e=i.error||"Failed to fetch consumption data";let o=e;throw i.warning&&(o+=`. ${i.warning}`),console.error("%c✗ Service returned error: %c"+e,"color: #f00; font-size: 11px; font-weight: bold;","color: #f00; font-size: 11px;"),console.log("%c  Requested Entry ID: %c"+t,"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),i.context&&(console.log("%c  Service Context: %c"+JSON.stringify(i.context,null,2),"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),i.context.id&&i.context.id!==t&&console.warn("%c⚠ Note: Service context shows different entry ID (%c"+i.context.id+"%c). This may be informational.","color: #ff9800; font-size: 11px;","color: #ff9800; font-size: 11px; font-family: monospace;","color: #ff9800; font-size: 11px;")),i.warning&&console.warn("%c⚠ Service Warning: %c"+i.warning,"color: #ff9800; font-size: 11px;","color: #ff9800; font-size: 11px;"),console.log("%c  Full Response: %c"+JSON.stringify({success:i.success,error:i.error,warning:i.warning,context:i.context},null,2),"color: #666; font-size: 11px;","color: #999; font-size: 11px; font-family: monospace;"),new Error(o)}if(this._consumptionData=i.consumption_data||[],this._comparisonError=null,this.config.show_tariff_comparison&&this.config.tariff_entry_ids&&this.config.tariff_entry_ids.length>0)try{const i="day"===this._currentPeriod?"daily":"week"===this._currentPeriod?"weekly":"monthly",r=await this._callServiceWithTimeout("octopus_energy_es","compare_tariffs",{tariff_entry_ids:this.config.tariff_entry_ids,source_entry_id:t,period:i,start_date:e.toISOString().split("T")[0],end_date:o.toISOString().split("T")[0]});if(r.success&&r.result)this._comparisonResult=r.result;else{const t=r.error||"Failed to compare tariffs";this._comparisonError=t,console.warn("%c⚠ Tariff comparison failed: %c"+t,"color: #ff9800; font-size: 11px;","color: #ff9800; font-size: 11px;")}}catch(t){const e=t instanceof Error?t.message:String(t);this._comparisonError=`Tariff comparison error: ${e}`,console.warn("%c⚠ Tariff comparison error: %c"+e,"color: #ff9800; font-size: 11px;","color: #ff9800; font-size: 11px;")}}catch(t){this._error=t instanceof Error?t.message:String(t),console.error("Error loading data:",t)}finally{this._loading=!1}}_getDateRange(){const t=new Date(this._currentDate);t.setHours(23,59,59,999);const e=new Date(t);return"day"===this._currentPeriod?e.setHours(0,0,0,0):"week"===this._currentPeriod?(e.setDate(e.getDate()-6),e.setHours(0,0,0,0)):"month"===this._currentPeriod&&(e.setDate(e.getDate()-29),e.setHours(0,0,0,0)),{startDate:e,endDate:t}}_navigatePeriod(t){const e="prev"===t?-1:1;"day"===this._currentPeriod?this._currentDate.setDate(this._currentDate.getDate()+e):"week"===this._currentPeriod?this._currentDate.setDate(this._currentDate.getDate()+7*e):"month"===this._currentPeriod&&this._currentDate.setMonth(this._currentDate.getMonth()+e),this._currentDate=new Date(this._currentDate),this._loadData()}_setPeriod(t){this._currentPeriod=t,this._loadData()}render(){return this._loading?F`
        <div class="loading">
